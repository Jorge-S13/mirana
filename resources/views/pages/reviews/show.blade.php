                ...
                <main class="post-grid col-md-9">
                    <div class="row">
                        <article class="post-item">
                            <p>{{$post->posted_at->format('d M, Y')}} <span><a href="{{route('reviews.categories',$post->category->slug)}}" class="view-btn">{{$post->category->name}}</a></span></p>
                        ...
                        <div class="widget mb-5">
                            ...
                            <ul class="list-unstyled bg-blue-trans p-3 rounded-3">
                                @foreach($categories as $category)
                                    <li>
                                        <a href="{{route('reviews.categories',$category->slug)}}" class="item-anchor">{{$category->name}}</a>
                                    </li>
                                    <hr class="m-0 mb-1">
                                @endforeach
                            </ul>
                        ...
                        <div class="widget mb-5">
                            ...
                            <ul class="list-unstyled bg-blue-trans p-3 rounded-3">
                                @foreach($recentPosts as $recentPost)
                                    <li>
                                        <a href="{{route('reviews.categories',$recentPost->category->slug)}}" class="view-btn">{{$recentPost->category->name}}</a>
                                        <a href="{{route('reviews.show',$recentPost->slug)}}"> <h3 class="block-title fs-6">{{$recentPost->title}}</h3> </a>
                                    </li>
                                    <hr class="m-0 mb-1">
                                @endforeach
                            </ul>
                            ...
                            <ul class="list-unstyled d-flex flex-wrap gap-2 bg-blue-trans p-3 rounded-3">
                                <li>
                                    @foreach($post->tags()->get() as $tag)
                                        <a href="{{route('reviews.tags',$tag->slug)}}" class="fs-6">{{ $tag->name . ',' }}</a>
                                    @endforeach
                                </li>
                            </ul>
                        </div>
                                ...
                                <div class="product-item bg-blue-trans p-3 rounded-1 mb-3 ">
                                    <div class="product-detail">
                                        <div class="clients-detail">
                                            <a href="{{route('reviews.show',$relatedPost->slug)}}"> <img src="{{asset('storage/' . $relatedPost->main_image)}}" alt="clients"> </a>
                                        </div>
                                        <div class="btn-card">
                                            <a href="{{route('reviews.categories',$relatedPost->category->slug)}}" class="view-btn">{{$relatedPost->category->name}}</a>
                                        </div>
                                        <a href="{{route('reviews.show',$relatedPost->slug)}}">
